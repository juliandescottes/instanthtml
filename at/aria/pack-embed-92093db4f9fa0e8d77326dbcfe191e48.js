...
//*******************
Aria.beanDefinitions({$package:"aria.embed.CfgBeans",$description:"Definition of the JSON beans used by the aria embed lib",$namespaces:{json:"aria.core.JsonTypes",html:"aria.templates.CfgBeans"},$beans:{ElementCfg:{$type:"json:Object",$description:"Embed element widget",$properties:{controller:{$type:"json:ObjectRef",$description:"Controller used to manage the embedded dom"},type:{$type:"json:String",$description:"DOM type for this section.",$default:"div"},attributes:{$type:"html:HtmlAttribute",$description
:"Parameters to apply to the DOM element of the section."},args:{$type:"json:MultiTypes",$description:"Argument given to the onEmbededElementCreate and onEmbededElementDispose functions of the provided embed controller"}}},PlaceholderCfg:{$type:"json:Object",$description:"Placeholder",$properties:{name:{$type:"json:String",$description:"Placeholder name",$mandatory:!0},type:{$type:"json:String",$description:"DOM type for this section.",$default:"div"},attributes:{$type:"html:HtmlAttribute",$description:"Parameters to apply to the DOM element of the section."
}}},MapCfg:{$type:"json:Object",$description:"Map widget configuration",$properties:{id:{$type:"json:String",$description:"Id of the map",$mandatory:!0},provider:{$type:"json:String",$description:"Map provider",$mandatory:!0},initArgs:{$type:"json:MultiTypes",$description:"Map initialization arguments"},loadingIndicator:{$type:"json:Boolean",$description:"Add a loading overlay over the map while loading",$default:!1},type:{$type:"json:String",$description:"DOM type for this section.",$default:"div"},attributes
:{$type:"html:HtmlAttribute",$description:"Parameters to apply to the DOM element of the section."}}}}});
//*******************
...
//*******************
Aria.interfaceDefinition({$classpath:"aria.embed.IContentProvider",$events:{contentChange:{description:"Raised when the content associated to one or more placeholder paths changes.",properties:{contentPaths:"{Array} contains the paths whose corresponding content has changed."}}},$interface:{getContent:function(e){}}});
//*******************
...
}}},$statics:{PLACEHOLDER_PATH_NOT_FOUND:"No content has been found for the placeholder path '%1'"},$prototype:{getContent:function(e){var t=[],n=aria.utils.Type,r=this._providers;for(var i=0,s=r.length;i<s;i++){var o=r[i],u=o.getContent(e);if(u)if(n.isArray(u))for(var a=0,f=u.length;a<f;a++)t.push(u[a]);else t.push(u)}return t.length===0&&this.$logWarn(this.PLACEHOLDER_PATH_NOT_FOUND,[e]),t},register:function(e){var t=this._providers;aria.utils.Array.contains(t,e)||(e.$addListeners({contentChange:this._contentChangeListener
}),t.push(e))},unregister:function(e){var t=this._providers;aria.utils.Array.remove(t,e)&&e.$removeListeners({contentChange:this._contentChangeListener})},unregisterAll:function(){var e=this._providers;while(e.length>0)this.unregister(e[0])},_onContentChange:function(e){this.$raiseEvent({name:"contentChange",placeholderPaths:e.contentPaths})}}});